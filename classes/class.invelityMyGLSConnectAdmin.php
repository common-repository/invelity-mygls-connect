...
class InvelityMyGLSConnectAdmin
{
    ...
    private function getRemoteAd()
    {
        ...
        if (!$invelityIkrosInvoicesad) {
            $query = esc_url_raw(add_query_arg([],
                'https://licenses.invelity.com/plugins/invelity-my-gls/invelityad.json'));
    ...
    function page_init()
    {
        ...
        add_settings_field(
            'custom_note',
            __('Note', $this->launcher->getPluginSlug()),
            [$this, 'custom_note_callback'],
            'invelity-mygls-connect-setting-admin',
            'setting_section_2'
        );
        ...
        add_settings_section(
            ...
            __('Email settings', $this->launcher->getPluginSlug()), // Title
            '__return_false',
            'invelity-mygls-connect-setting-admin' // Page
        );
        add_settings_field(
            'allow_tracking_email',
            __('Allow tracking email', $this->launcher->getPluginSlug()),
            [$this, 'allow_tracking_email'],
            'invelity-mygls-connect-setting-admin',
            'setting_section_5'
        );

        add_settings_field(
            'tracking_email_subject_settings',
            __('Tracking Email Subject', $this->launcher->getPluginSlug()),
            [$this, 'tracking_email_subject_settings_callback'],
            'invelity-mygls-connect-setting-admin',
            'setting_section_5'
        );

        //add settings field tinymce_editor_settings for tinymce editor
        add_settings_field(
            'tracking_email_text_settings',
            __('Tracking Email text', $this->launcher->getPluginSlug()),
            [$this, 'tracking_email_text_settings_callback'],
            'invelity-mygls-connect-setting-admin',
            'setting_section_5'
        );
    }
    ...
    public function sanitize($input)
    {
        ...
        if (isset($input['custom_note'])) {
            if (reset($input['custom_note']) == 'custom') {
                $new_input['custom_note'] = sanitize_text_field(end($input['custom_note']));
            } else {
                $new_input['custom_note'] = sanitize_text_field(reset($input['custom_note']));
            }
        }
        ...
        if (isset($input['allow_tracking_email'])) {
            $new_input['allow_tracking_email'] = sanitize_text_field($input['allow_tracking_email']);
        }

        if (isset($input['tracking_email_subject_settings'])) {
            $new_input['tracking_email_subject_settings'] = sanitize_text_field($input['tracking_email_subject_settings']);
        }
        if (isset($input['tracking_email_text_settings'])) {
            $new_input['tracking_email_text_settings'] = $input['tracking_email_text_settings'];
        }
    ...
    public function country_version_callback()
    {
    ...
    public function sender_contact_email_callback()
    {
...
//    }
    ...
    public function clientref_callback()
    {
        ...
        printf(
            '<input 
                type="text" 
                id="clientref" 
                name="invelity_my_gls_export_options[clientref]" 
                value="%s"
            /><p>Default value is order ID</p>',
            isset($this->options['clientref']) ? esc_attr($this->options['clientref']) : ''
        );
    }

    public function fds_callback()
    {
    ...
    public function custom_note_callback()
    {
        //multiple radio buttons with value note,order_number,custom.. if custom then show input type text
        printf(
            '<input type="radio" id="custom_note" name="invelity_my_gls_export_options[custom_note][]" %s  value="wc_note" /><label for="">Customer note</label><br>',
            isset($this->options['custom_note']) && $this->options['custom_note'] == 'wc_note' ? 'checked' : ''
        );
        printf(
            '<input type="radio" id="custom_note" name="invelity_my_gls_export_options[custom_note][]" %s  value="order_number" /><label for="">Order number</label><br>',
            isset($this->options['custom_note']) && $this->options['custom_note'] == 'order_number' ? 'checked' : ''
        );
        printf(
            '<input type="radio" id="custom_note" name="invelity_my_gls_export_options[custom_note][]" %s  value="custom" /><label for="">Custom</label><br>',
            isset($this->options['custom_note']) && $this->options['custom_note'] !== 'order_number' && $this->options['custom_note'] !== 'wc_note' ? 'checked' : ''
        );
        printf(
            '<input type="text" id="custom_note" name="invelity_my_gls_export_options[custom_note][]" value="%s" />',
            isset($this->options['custom_note']) ? esc_attr($this->options['custom_note']) : ''
        );

    }

    public function allow_tracking_email()
    {

        //checkbox for tracking email
        printf(
            '<input type="checkbox" id="allow_tracking_email" name="invelity_my_gls_export_options[allow_tracking_email]" %s />',
            isset($this->options['allow_tracking_email']) && ($this->options['allow_tracking_email'] == 'on') ? 'checked' : ''
        );
    }

    public function tracking_email_text_settings_callback()
    {

        //printf wysiwig editor for textarea
        wp_editor(
            $this->options['tracking_email_text_settings'] ?? '',
            'tracking_email_text_settings',
            [
                'textarea_name' => 'invelity_my_gls_export_options[tracking_email_text_settings]',
                'textarea_rows' => 10,
                'editor_height' => 230,
                'teeny' => true,
                'quicktags' => false,
                'editor_class' => 'no-editor-class tracking_email_text_settings',
            ]
        );

        echo '<p>Please use the following tags to insert the tracking link, name, order number: <code>{tracking_link}</code>, <code>{site_url}</code>, <code>{site_title}</code>, <code>{name}</code>, and <code>{order_number}</code></p>';

    }

    public function tracking_email_subject_settings_callback()
    {

        printf(
            '<input type="text" id="tracking_email_subject_settings" name="invelity_my_gls_export_options[tracking_email_subject_settings]" value="%s" />',
            isset($this->options['tracking_email_subject_settings']) ? esc_attr($this->options['tracking_email_subject_settings']) : ''
        );
    }
}